...
const ThreadHistorySkeleton: FC = () => {
  ...
  >
    <span className="sr-only">{pick({ tr: "Sohbet yükleniyor", en: "Loading conversation" })}</span>
      ...
      >
        <div
          className={cn(
            ...
            isEmpty && "justify-start pt-6 md:justify-center md:pt-4",
          )}
        >
...
const Composer: FC<{ autoFocus: boolean }> = ({ autoFocus }) => {
  ...
  return (
    <ComposerPrimitive.Root className="aui-composer-root relative flex w-full flex-col">
      <ComposerPrimitive.AttachmentDropzone render={<div data-slot="aui_composer-shell" className="border-border/60 data-[dragging=true]:border-ring focus-within:border-primary/35 focus-within:shadow-[0_0_0_4px_color-mix(in_oklab,var(--primary)_9%,transparent),0_18px_48px_rgb(50_35_25/10%)] dark:border-muted-foreground/15 dark:focus-within:border-primary/45 dark:focus-within:shadow-[0_0_0_4px_color-mix(in_oklab,var(--primary)_14%,transparent),0_20px_54px_rgb(0_0_0/35%)] flex w-full cursor-text flex-col gap-2 rounded-(--composer-radius) border bg-(--composer-bg) p-(--composer-padding) transition-[border-color,box-shadow,transform] duration-300 focus-within:-translate-y-0.5 data-[dragging=true]:border-dashed data-[dragging=true]:bg-[color-mix(in_oklab,var(--color-accent)_50%,var(--color-background))]" />}><ComposerAttachments /><ComposerPrimitive.Input
                      placeholder={pick({ tr: "Dersler, kampüs veya ödevlerin hakkında sor…", en: "Ask about courses, campus, or homework…" })}
                      className="aui-composer-input caret-primary placeholder:text-muted-foreground max-h-48 min-h-10 w-full resize-none bg-transparent px-2.5 py-1 text-base leading-6 outline-none"
                      rows={1}
                      autoFocus={autoFocus}
                      enterKeyHint="send"
      ...
      <div className="flex items-center gap-1.5">
        <AuiIf condition={(s) => s.thread.capabilities.dictation}>
          <AuiIf condition={(s) => s.composer.dictation == null}>
            <ComposerPrimitive.Dictate render={<TooltipIconButton tooltip={pick({ tr: "Sesle yaz", en: "Voice input" })} side="bottom" type="button" variant="ghost" size="icon" className="aui-composer-dictate text-muted-foreground hover:text-foreground size-7 rounded-full" aria-label={pick({ tr: "Sesle yazmayı başlat", en: "Start voice input" })} />}><MicIcon className="aui-composer-dictate-icon size-4" /></ComposerPrimitive.Dictate>
          </AuiIf>
          <AuiIf condition={(s) => s.composer.dictation != null}>
            <ComposerPrimitive.StopDictation render={<TooltipIconButton tooltip={pick({ tr: "Sesle yazmayı durdur", en: "Stop dictation" })} side="bottom" type="button" variant="ghost" size="icon" className="aui-composer-stop-dictation text-destructive size-7 rounded-full" aria-label={pick({ tr: "Sesle yazmayı durdur", en: "Stop voice input" })} />}><SquareIcon className="aui-composer-stop-dictation-icon size-3.5 animate-pulse fill-current" /></ComposerPrimitive.StopDictation>
          </AuiIf>
        </AuiIf>
...
const AssistantActionBar: FC = () => {
  ...
  return (
    ...
    >
      <ActionBarPrimitive.Copy render={<TooltipIconButton tooltip={pick({ tr: "Kopyala", en: "Copy" })} />}><AuiIf condition={(s) => s.message.isCopied}>
                      <CheckIcon className="animate-in zoom-in-50 fade-in duration-200 ease-out" />
                    </AuiIf><AuiIf condition={(s) => !s.message.isCopied}>
                      <CopyIcon className="animate-in zoom-in-75 fade-in duration-150" />
                    </AuiIf></ActionBarPrimitive.Copy>
      <ActionBarPrimitive.Reload render={<TooltipIconButton tooltip={pick({ tr: "Yeniden yanıtla", en: "Refresh" })} />}><RefreshCwIcon /></ActionBarPrimitive.Reload>
      <ActionBarMorePrimitive.Root>
        <ActionBarMorePrimitive.Trigger render={<TooltipIconButton tooltip={pick({ tr: "Diğer işlemler", en: "More" })} className="data-[state=open]:bg-accent" />}><MoreHorizontalIcon /></ActionBarMorePrimitive.Trigger>
        ...
        >
          <ActionBarPrimitive.ExportMarkdown render={<ActionBarMorePrimitive.Item className="aui-action-bar-more-item hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground flex cursor-pointer items-center gap-2 rounded-lg px-2.5 py-1.5 text-sm outline-none select-none" />}><DownloadIcon className="size-4" />{pick({ tr: "Markdown olarak indir", en: "Export as Markdown" })}
...
const UserActionBar: FC = () => {
  ...
  return (
    ...
    >
      <ActionBarPrimitive.Edit render={<TooltipIconButton tooltip={pick({ tr: "Mesajı düzenle", en: "Edit" })} className="aui-user-action-edit" />}><PencilIcon /></ActionBarPrimitive.Edit>
    </ActionBarPrimitive.Root>
  );
};
    ...
    >
      <ComposerPrimitive.Root className="aui-edit-composer-root border-border/60 dark:border-muted-foreground/15 ms-auto flex w-full max-w-[85%] cursor-text flex-col rounded-(--composer-radius) border bg-(--composer-bg)">
        ...
        <div className="aui-edit-composer-footer mx-2.5 mb-2.5 flex items-center gap-1.5 self-end">
          <ComposerPrimitive.Cancel render={<Button variant="ghost" size="sm" className="h-8 rounded-full px-3.5" />}>{pick({ tr: "Vazgeç", en: "Cancel" })}
                              </ComposerPrimitive.Cancel>
          <ComposerPrimitive.Send render={<Button size="sm" className="h-8 rounded-full px-3.5" />}>{pick({ tr: "Kaydet ve yeniden sor", en: "Update" })}
...
const BranchPicker: FC<BranchPickerPrimitive.Root.Props> = ({
  ...
  return (
    <BranchPickerPrimitive.Root
      ...
      {...rest}
    >
      <BranchPickerPrimitive.Previous render={<TooltipIconButton tooltip={pick({ tr: "Önceki yanıt", en: "Previous" })} />}><ChevronLeftIcon /></BranchPickerPrimitive.Previous>
      <span className="aui-branch-picker-state font-medium">
        <BranchPickerPrimitive.Number /> / <BranchPickerPrimitive.Count />
      </span>
      <BranchPickerPrimitive.Next render={<TooltipIconButton tooltip={pick({ tr: "Sonraki yanıt", en: "Next" })} />}><ChevronRightIcon /></BranchPickerPrimitive.Next>
    </BranchPickerPrimitive.Root>
  );
};
