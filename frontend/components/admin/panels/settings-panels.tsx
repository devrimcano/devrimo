...
function RuntimeForm({ initial }: { initial: RuntimeSettings }) {
  ...
  const [reason, setReason] = useState("");
  const editableFields = useMemo(() => ({ model_id: form.model_id, max_tokens: form.max_tokens, scholar_history_runs: form.scholar_history_runs, tool_call_limit: form.tool_call_limit, learning_enabled: form.learning_enabled, input_token_price: form.input_token_price, output_token_price: form.output_token_price }), [form]);
  const initialFields = useMemo(() => ({ model_id: initial.model_id, max_tokens: initial.max_tokens, scholar_history_runs: initial.scholar_history_runs, tool_call_limit: initial.tool_call_limit, learning_enabled: initial.learning_enabled, input_token_price: initial.input_token_price, output_token_price: initial.output_token_price }), [initial]);
  const valid = form.model_id.trim().length >= 2 && form.max_tokens >= 256 && form.max_tokens <= 131072 && form.scholar_history_runs >= 0 && form.scholar_history_runs <= 50 && form.tool_call_limit >= 1 && form.tool_call_limit <= 50 && form.input_token_price >= 0 && form.input_token_price <= 1 && form.output_token_price >= 0 && form.output_token_price <= 1;
  ...
  const numberField = (key: keyof Pick<RuntimeSettings, "max_tokens" | "scholar_history_runs" | "tool_call_limit" | "input_token_price" | "output_token_price">, value: number) => setForm((current) => ({ ...current, [key]: value }));
