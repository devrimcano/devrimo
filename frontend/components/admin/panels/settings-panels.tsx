...
function RuntimeForm({ initial }: { initial: RuntimeSettings }) {
  ...
  const [reason, setReason] = useState("");
  const editableFields = useMemo(() => ({ model_id: form.model_id, max_tokens: form.max_tokens, scholar_history_runs: form.scholar_history_runs, tool_call_limit: form.tool_call_limit, learning_enabled: form.learning_enabled, input_token_price: form.input_token_price, output_token_price: form.output_token_price, rate_limit_enabled: form.rate_limit_enabled, rate_limit_chat_per_minute: form.rate_limit_chat_per_minute, rate_limit_catalog_per_minute: form.rate_limit_catalog_per_minute }), [form]);
  const initialFields = useMemo(() => ({ model_id: initial.model_id, max_tokens: initial.max_tokens, scholar_history_runs: initial.scholar_history_runs, tool_call_limit: initial.tool_call_limit, learning_enabled: initial.learning_enabled, input_token_price: initial.input_token_price, output_token_price: initial.output_token_price, rate_limit_enabled: initial.rate_limit_enabled, rate_limit_chat_per_minute: initial.rate_limit_chat_per_minute, rate_limit_catalog_per_minute: initial.rate_limit_catalog_per_minute }), [initial]);
  const valid = form.model_id.trim().length >= 2 && form.max_tokens >= 256 && form.max_tokens <= 131072 && form.scholar_history_runs >= 0 && form.scholar_history_runs <= 50 && form.tool_call_limit >= 1 && form.tool_call_limit <= 50 && form.input_token_price >= 0 && form.input_token_price <= 1 && form.output_token_price >= 0 && form.output_token_price <= 1 && form.rate_limit_chat_per_minute >= 0 && form.rate_limit_chat_per_minute <= 100000 && form.rate_limit_catalog_per_minute >= 0 && form.rate_limit_catalog_per_minute <= 100000;
  ...
  return <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
    <Card className="surface-raised border-0 ring-1 ring-foreground/8"><CardHeader className="border-b"><div className="flex items-start justify-between gap-3"><div><CardTitle>{pick({ tr: "Çalışma zamanı varsayılanları", en: "Runtime defaults" })}</CardTitle><CardDescription className="mt-1">{pick({ tr: "Yeni istekler kaydedilen ayarları kullanır; sıradaki isteklerin ayarları korunur.", en: "New requests use the saved defaults; queued requests retain their configuration." })}</CardDescription></div>{dirty ? <Badge>{pick({ tr: "Kaydedilmemiş", en: "Unsaved" })}</Badge> : <Badge variant="outline">{pick({ tr: "Güncel", en: "Up to date" })}</Badge>}</div></CardHeader><CardContent><form className="space-y-5" onSubmit={(event: FormEvent) => { event.preventDefault(); mutation.mutate(); }}>
      ...
      <div className="border-t pt-5"><p className="mb-3 text-xs font-semibold tracking-[0.12em] text-muted-foreground uppercase">{pick({ tr: "Hız sınırları", en: "Rate limits" })}</p><div className="grid gap-4 sm:grid-cols-2"><label className="flex items-center justify-between gap-3 rounded-xl border bg-muted/25 p-3 sm:col-span-2"><span><span className="block font-medium">{pick({ tr: "İstek sınırlarını uygula", en: "Enforce request limits" })}</span><span className="mt-0.5 block text-xs text-muted-foreground">{pick({ tr: "Altyapı hazır; kapalıyken hiçbir istek sınırlanmaz. Açıldığında sınırlar hesap başına dakikada uygulanır (0 = sınırsız).", en: "The mechanism is ready; while off, no request is limited. When on, limits apply per account per minute (0 = unlimited)." })}</span></span><Switch checked={form.rate_limit_enabled} onCheckedChange={(checked) => setForm({ ...form, rate_limit_enabled: checked })} /></label><NumberField id="rate-limit-chat" label={pick({ tr: "Sohbet istekleri (dakikada)", en: "Chat requests (per minute)" })} value={form.rate_limit_chat_per_minute} min={0} max={100000} onChange={(value) => numberField("rate_limit_chat_per_minute", value)} /><NumberField id="rate-limit-catalog" label={pick({ tr: "Katalog okumaları (dakikada)", en: "Catalog reads (per minute)" })} value={form.rate_limit_catalog_per_minute} min={0} max={100000} onChange={(value) => numberField("rate_limit_catalog_per_minute", value)} /></div></div></fieldset>
...
function SystemContent({ data, principal, sync, syncing }: { data: SystemHealth; principal: AdminPrincipal; sync: () => void; syncing: boolean }) {
  ...
  const capacity = data.integration_capacity > 0 ? Math.round((data.integration_sessions / data.integration_capacity) * 100) : 0;
