...
def test_timetable_solver_skips_ineligible_and_empty_day_sections():
    ...
    # Required pool courses are solved as one complete plan.  Returning the
    # feasible PHYS101 fragment would make the UI look successful while
    # silently dropping MATH101, so the owner reports the unschedulable pool
    # instead and preserves the previous entries.
    assert solved.entries == []
    assert solved.alternatives == []
    assert solved.unscheduled_courses == ["MATH101"]
    assert solved.generation_error
