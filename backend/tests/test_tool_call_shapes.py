...
from app.workspace.resources import ResourceRef, SearchResource, normalize_term, scoped_ref
...
def test_flat_fields_build_the_same_reference_as_the_nested_object():
    flat = scoped_ref(ResourceRef, "read", None, "catalog.sections", key="CENG 331", term="2026-2027 Fall")
    nested = scoped_ref(
    ...
    search = scoped_ref(SearchResource, "search", None, "researcher", department="CENG")
...
def test_a_flat_call_without_a_kind_is_told_what_is_missing():
    with pytest.raises(HTTPException) as excinfo:
        scoped_ref(ResourceRef, "read", None, None, key="CENG 331")
