...
def normalize_sections(value: Any) -> list[dict[str, Any]]:
    ...
    for index, record in enumerate(_records(value)):
        ...
        raw_status = _value(record, ("eligibility_status", "eligibilityStatus", "status"))
        eligibility_status = str(raw_status or "unverified").strip().lower()
        if eligibility_status not in {"verified", "stale", "unavailable", "unverified"}:
        ...
        # Preserve the historical raw catalog shape when no student verdict is
        # attached. The typed API supplies defaults, while older callers and
        # tests that compare catalog-only normalization remain compatible.
        if isinstance(record.get("eligible"), bool):
            row["eligible"] = record["eligible"]
        if raw_status not in (None, ""):
            row["eligibility_status"] = eligibility_status
        raw_reason = _value(record, ("reason", "eligibility_reason", "eligibilityReason"))
        if raw_reason not in (None, ""):
            row["reason"] = str(raw_reason).strip()
        raw_token = _value(record, ("eligibility_token", "eligibilityToken"))
        if raw_token not in (None, ""):
            row["eligibility_token"] = str(raw_token).strip()
        rows.append(row)
    return rows
