...
def build_agent(user_id: UUID, runtime: AgentRuntimeConfig) -> Agent:
    ...
    if profile == "legacy":
        return build_legacy_agent(user_id, runtime)
    if profile == "scholar":
        from app.agents.scholar.build import build_scholar_agent

        return build_scholar_agent(runtime, user_id=user_id)
