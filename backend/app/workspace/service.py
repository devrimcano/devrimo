...
class WorkspaceService:
    ...
    async def _annotate_proposal_eligibility(self, proposal: dict, term: str) -> None:
        """Attach private, server-owned section verdicts to an assistant plan.

        ``plan_semester`` optimizes transcript prerequisites and catalog
        offerings, but a normal timetable write also needs the current
        student's section eligibility. Fetching that verdict here keeps the
        assistant on the same tri-state contract as the schedule UI. The
        proposal contains only status/reason fields; raw transcript and
        eligibility rows stay inside the owner service.
        """
        courses = proposal.get("courses")
        ...
        for course in courses:
                ...
                verdicts.append(
                    {
                        ...
                        "reason": "The selected course has no complete section identity.",
                    }
                )
                continue
            try:
                ...
                # An upstream adapter failure is still an unknown result. Do
                # not turn an unexpected exception into a normal-plan allow.
                result = {
                    "eligible": None,
                    "eligibility_status": "unavailable",
                    "reason": "Eligibility data is temporarily unavailable",
            ...
            status = str(result.get("eligibility_status") or "unverified").lower()
            if status not in {"verified", "stale", "unavailable", "unverified"}:
                status = "unverified"
            verdicts.append(
                {
                    "course_code": code,
                    "section": section,
                    "eligible": result.get("eligible") if status == "verified" else None,
                    "eligibility_status": status,
                    "reason": str(result.get("reason") or "").strip(),
                    "eligibility_token": result.get("eligibility_token") if status == "verified" else None,
                    "eligibility_course_code": result.get("eligibility_course_code"),
                    "eligibility_raw_code": result.get("eligibility_raw_code"),
                }
            )
        proposal["section_verdicts"] = verdicts
        if all(item["eligibility_status"] == "verified" for item in verdicts):
            proposal["eligibility_status"] = "verified"
        elif any(item["eligibility_status"] == "unavailable" for item in verdicts):
            proposal["eligibility_status"] = "unavailable"
        else:
            proposal["eligibility_status"] = "unverified"
    ...
    async def plan(self, request: dict):
        ...
        async with SessionLocal() as db:
            ...
            await self._annotate_proposal_eligibility(proposal, parsed.term)
            changes = proposal_changes(proposal, require_verified=True)
